...
export default function ContentModalAreaCreate({name, onClickAplicar, onClickFechar, onChangeCreateArea}){
        return(
            <div className="w-[50%] h-[36%] bg-[#242424] border border-[#474747] rounded-xl">
                ...
                <div className="mx-10 mr-24">
                    <div className="flex flex-col my-12 gap-y-5">
                        ...
                        <input type="text" className="bg-[#3A3A3A] py-4 rounded px-3 text-lg text-[#C4C4C4] font-medium" onChange={onChangeCreateArea}/>
                    </div>
    
                    <div className=" flex justify-end gap-x-5">
                        <ButtonCinza label={'cancelar'} onClick={()=>onClickFechar()} />
                        <ButtonPink label={'criar'} hasIcon={false} onClick={()=>onClickAplicar()} />
                    </div>
                </div>
